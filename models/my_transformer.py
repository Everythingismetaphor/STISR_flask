...
class TPTransformer(nn.Module):
    ...
    def forward(self, src, cnt_memory, mask, cnt_mask, query_embed, pos_embed, cnt_pos_embed, tgt=None,
                text_prior=None):
        ...
        hs = self.decoder(tgt, global_memory, cnt_memory, memory_key_padding_mask=mask,
                          cnt_memory_key_padding_mask=cnt_mask,
...
class InfoTransformer(nn.Module):
    ...
    def __init__(self, d_model=1024, nhead=8, num_encoder_layers=3,
        ...
        # 1024
        self.gru_encoding = nn.GRU(d_model * feat_height, d_model * feat_height // 2, bidirectional=True,
                                   batch_first=True)
    ...
    def forward(self, src, mask, query_embed, pos_embed, tgt=None, text_prior=None, spatial_size=(16, 64)):
        ...
        query_embed, _ = self.gru_encoding(query_embed)  #gru位置编码
        ...
        memory = self.encoder(src,img=tgt, src_key_padding_mask=mask, pos=pos_embed,query_pos=query_embed)  # 编码器  又加入tgt图片信息
        ##去掉解码器
        #hs = self.decoder(tgt, memory, memory_key_padding_mask=mask,  # 解码器
        #                  pos=pos_embed, query_pos=query_embed)
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src,img: Optional[Tensor] = None,
        ...
        for layer in self.layers:
            if type(src) == list:
                ...
                src_item = src
            output = layer(output + src_item, image=img,src_mask=mask,
                           src_key_padding_mask=src_key_padding_mask, pos=pos,query_pos=query_pos)
...
class TransformerDualDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, sr_target_decoder, num_layers, norm=None, return_intermediate=False, height=16,
                 width=64):
        super().__init__()
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.self_attn_mca = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
    ...
    # 在self-attention之后应用了LayerNorm   用的这个！
    def forward_post(self,
                     src,image: Optional[Tensor] = None,
        ...
        #src = self.self_attn_mca(src,self.with_pos_embed(image, query_pos),self.with_pos_embed(image, query_pos))[0] q,k,v顺序写错了
        src = self.self_attn_mca(self.with_pos_embed(image, query_pos), src,  src)[0]
        src = src1 + self.dropout3(src)
        src = self.norm3(src)
        ##FFN
    ...
    def forward(self, src,
        ...
        return self.forward_post(src, image,src_mask, src_key_padding_mask, pos,query_pos)
